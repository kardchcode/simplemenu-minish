...
void drawSettingsScreen() {
	...
	TIDY_ROMS_OPTION=3;
	FULL_SCREEN_FOOTER_OPTION=7;
	FULL_SCREEN_MENU_OPTION=8;
	DEFAULT_OPTION=4;
	...
	drawRectangleToScreen(SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, bodyBackground);
	...
	int nextLine = calculateProportionalSizeOrDistance1(20);
	int nextLineText = calculateProportionalSizeOrDistance1(20);
	int selected=0;
	...
	for (int i=0;i<max;i++) {
		...
		if(i==chosenSetting) {
			...
			drawShadedSettingsOptionOnScreen(options[i], nextLineText, bodyBackground, bodyHighlightedText);
			drawShadedSettingsOptionValueOnScreen(options[i],values[i], nextLineText, bodyHighlightedText, bodyBackground);
			selected=i;
		} else {
		...
		nextLine+=calculateProportionalSizeOrDistance1(30);
		logMessage("INFO","drawSettingsScreen","Next line 2");
		nextLineText+=calculateProportionalSizeOrDistance1(30);
